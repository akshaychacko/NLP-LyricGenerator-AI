...
###############################################################################
    ...
    def selectNGramModel(self, sentence):
        ...
        if (tri.trainingDataHasNGram(sentence) == True):
            return tri
        if (bi.trainingDataHasNGram(sentence) == True):
            return bi
        else:
            return uni
    ...
    def weightedChoice(self, candidates):
        ...
        for member in candidates:
            value = candidates[member]
            cumulative += value
            candidates[member] = cumulative
            
        x = random.randrange(0, cumulative)
    ...
    def getNextToken(self, sentence, filter=None):
        ...
        if filter==None:
            nextToken = self.weightedChoice(candidateDictionary)
        if filter != None:
            for token in candidateDictionary:
                if token in filter:
                    filterCandidates[token] = count
            if filterCandidates:
                nextToken = self.weightedChoice(filteredCandidates)
            if not filterCandidates:
                nextToken = random.choice(list(filter))
...
if __name__ == '__main__':
    ...
    # Dictionary = {"red": 6, "green": 2, "blue": 4}
    # x = lang.selectNGramModel(Dictionary)
    # print (x) #get rid of it eventually
    
    # x = lang.weightedChoice(Dictionary)
    # print (x) #get rid of it eventually
    
    # getNextToken test w/ unigram code
    # text = [ ['My', 'favorite', 'color', 'is', 'red'], ['My', 'favorite', 'color', 'is', 'blue']]
    # lang.updateTrainedData(text, prepped=True)
    # sentence = "My favorite color is "
    # x = lang.getNextToken(sentence, filter=None)
    # print (sentence + x)
